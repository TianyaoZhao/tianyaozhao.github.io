...
![image-20240630124611249](./.assets/image-20240630124611249.png)

**论文思想：**

1. 将一副图像分成**S×S**个网格，如果**某个object的中心落在这个网格**，这个网格就**负责**预测这个object

    ![image-20240630124344655](./.assets/image-20240630124344655.png)

2. 每个网格负责预测**B**个bbx，每个bbx除了**预测位置**之外，还要预测一个**confidence**（IOU），每个网格还要预测**C个类别**的分数



**损失函数：**

![image-20240630124750018](./.assets/image-20240630124750018.png)

**存在问题**

1. 群体小目标预测效果差，一个格子预测2个bbx

### yolov2

1. BN：在每个卷积层后添加了BN层，帮助模型起到正则化的作用，舍弃dropout层
2. 高分辨率分类器
3. 使用基于Anchor的目标边界框预测，预测偏移量
4. Anchor的聚类，defaultBox
5. 目标边界框预测，让某一个anchor只负责预测当前区域内的目标
6. 高层信息和低层信息进行融合
7. 多尺度训练策略：每迭代10个batch进行输入尺寸的随机修改

### yolov3

![image-20240630131254050](./.assets/image-20240630131254050.png)

1. 修改backbone为darknet53

2. 在三个预测特征层上进行预测，每个特征层对应不同的边界框

    ![image-20240630131127693](./.assets/image-20240630131127693.png)

### yolov4

**网络结构：**

![image-20240630131721658](./.assets/image-20240630131721658.png)

BackBone：CSPDarknet53    CSP：加快速度，减少显存，提高学习能力

![image-20240630131907590](./.assets/image-20240630131907590.png)

![image-20240630131938414](./.assets/image-20240630131938414.png)

Neck：SPP（解决多尺度问题），PAN（高层向底层融合+底层向高层融合）
...
![在这里插入图片描述](./.assets/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L1daWjE4MTkxMTcxNjYx,size_16,color_FFFFFF,t_70#pic_center.jpeg)

1. 输入：数据增强 采用了4张图片，并且按照随机缩放、随机裁剪和随机排布的方式进行拼接而成 和自适应的锚框计算（**YOLOv5中将此功能嵌入到代码中，每次训练时，根据数据集的名称自适应的计算出最佳的锚点框**）自适应缩放：**自适应的添加最少的黑边到缩放之后的图片中**
2. backbone：融合其他网络改进 比如CSP比如FOCUS
3. Neck：在BackBone中插入Neck网络，比如FPN+PAN
